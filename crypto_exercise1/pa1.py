import string

def findkey(subarr):
    chars = string.ascii_letters+string.digits+','+'.'+' '
    keys = []
    for i in range(0x00, 0xff):
        keys.append(i)
        for c in subarr:
            if chr(c^i) not in chars:
                keys.remove(i)
                break
    return keys

cipher = ''
arr = []
for i in range(0, len(cipher), 2):
    arr.append(int(cipher[i:2+i], 16))

for keylen in range(1, 14):
    for index in range(0, keylen):
        subarr = arr[index::keylen]
        keys = findkey(subarr)
        print 'keylen = {} index = {} keys = {}'.format(keylen, index, keys)

key = []
for index in range(0, 7):
    subarr = arr[index::7]
    key.append(findkey(subarr))

plaintext = ''
for i in range(0, len(arr)):
    plaintext += chr(arr[i]^key[i%7][0])
print(plaintext)
